...
namespace ParsElements.Services
{
    internal class Scraper
    {
        public static async Task AvitoApartment(DatabaseManager db, Logger logger)
        {
            ...
            try
            {
                ...
                {
                    logger.Error("Не получилось нажать на кнопку поиск");
                }

                for (int p = 1; p <= 5; p++)
                {
                    logger.Info("Смотрим страницу {Page}", p);
                    ...
                    foreach (var listing in listings)
                    {
                        ...
                        {
                            logger.Warn("Error parsing element: {Message}", ex.Message);
                        }
                    }
            ...
            {
                logger.Error(ex, "Scraper crashed");
            }
        }
        static async Task MarketPhone(DatabaseManager db, Logger logger)
        {
            using var playwright = await Playwright.CreateAsync();
            // Настройка браузера
            await using var browser = await playwright.Chromium.LaunchAsync(new BrowserTypeLaunchOptions
            ...
            {
                logger.Fatal(ex, "Scraper crashed");
            }
        }
    }
}
